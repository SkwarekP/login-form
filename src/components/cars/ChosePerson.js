...
export const ChosePerson = (props)=>{
    ...
    return (<p>
        ...
        {osoby.map((o)=>props.osoba!==null && o.id===props.osoba.id ? 
            <option key={o.user_id} selected>{o.imie} {o.nazwisko}</option> : 
            <option key={o.user_id}>{o.imie} {o.nazwisko}</option>)}
        </select>
        <button onClick={()=>props.submitChange(chosenPerson)}>Zatwierdz</button> 
    </p>)
}